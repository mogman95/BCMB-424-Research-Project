...
### This dialog is presented when more or less than exactly 2 rows are slected
    def __init__(self):                                         # Initialize class
        ...
        uic.loadUi(os.getcwd() + '/ui_files/km2rows-dialog.ui', self)                     # Load the .ui file
...
# processed correctly before continuing
    def __init__(self):
        ...
        uic.loadUi(os.getcwd() + '/ui_files/kmdata-conf.ui', self)
...
# to confirm that the data has been cleaned correctly
    def __init__(self):
        ...
        uic.loadUi(os.getcwd() + '/ui_files/kmdata-selection.ui', self)
        self.setStyleSheet('QMainWindow{background-color: white;}')

        ### Creates instances of the other necessary windows and pop-ups
        self.rerdialog = rows_errDialog()
        ...
        self.colmap.format_btn.clicked.connect(self.format)          # Prompts the user to save a file of the formatted data
        self.colmap.back_btn.clicked.connect(self.back)              # Goes back to the data selection window and hides the column map window
    ...
    def selectRows(self):
        ...
        try:
            ...
            self.rerdialog.show()                        # Prompt error pop-up
    ...
    def finDataProc(self):
        ...
        self.colmap.initial_timeunits_label.setText(self.timeunits)     # Shows the user the identified time units in the column map selection window
        self.headers = self.datafin.iloc[0, :]
        self.dataToTable(self.datafin, self.dataconf.conf_table) # Transfer the final data to the confirmation table
    ...
    def submit(self):
        ...
        if self.Bslctc_label.isVisible():  # if the label associated with the column selection step is visible...
            ...
            self.dataconf.show()           # Present data confirmation dialog
            self.dataconf.yes_btn.clicked.connect(
        ...
        if not(self.rerdialog.isVisible()) and not(self.Bslctc_label.isVisible()): # if the row error dialog isn't present
    ...
    def hidetransfer(self):
        for h, head in enumerate(self.headers):                                  # for the headers of the data...
            self.colmap.headers_table.setItem(0, h, QTableWidgetItem(str(head))) # Transfer them to the headers table
        self.dataconf.hide() # Hide the data confirmation dialog
        self.hide()          # Hide the data selection window
        self.colmap.showMaximized()   # Show the column map selection window
    ...
    def format(self):
        ...
        for col in col_range: # for each of the columns...
            ...
            while str(self.colmap.maintable.item(row, col)) != 'None':               # while the cell is not empty...
                colmapdf.loc[row, col] = self.colmap.maintable.item(row, col).text() # Transfer the contents of the main column map table to a dataframe
                row += 1                                                             # Move to the next row number

        concunits = self.colmap.conc_combobox.currentText() # Gets concentration units
        totime = self.colmap.to_combobox.currentText()      # Gets the time units to convert to
        ...
        try:
            ...
            name = QFileDialog.getSaveFileName(self, caption='Save As...', # Set the window caption
                                               directory=os.getcwd(),      # Set the working directory
            ...
            abtoconcf = self.colmap.abs_to_conc_spinbox.value() # Get absorbance to concentration conversion factor defined by user
            ### Set conversion factors dataframe (used to convert units in analysis)

            convfactorsdf = pd.DataFrame([['name', 'value', 'units'],
                                          ['absorbance2concentration', f'{abtoconcf}', f'{concunits}'],
                                          ['time_conversion', convf[f'Seconds to {self.timeunits}'], f'{self.timeunits}/Seconds']])
...
# [Enzyme]: Enzyme concentration
    def __init__(self):
        ...
        uic.loadUi(os.getcwd() + '/ui_files/kmcolumn-map-selection.ui', self)
        self.setStyleSheet('QMainWindow{background-color: white;}')
...
window = DataSelection()                # Create an instance of our class
