...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def gather_args(self):
        ...
        if self.output_edit.text() == "":
            ...
            self.output_path = self.input_path.strip(".xlsx") + "-output.xlsx"
...
#        self.input_path = "/home/ddooley/software/KmCalc/examples/1/ex1.xlsx"
        ...
        myFile = QtWidgets.QFileDialog.getOpenFileName(filed, "Please select an input file (XLSX)...")
        if (myFile[0] != ""):
            if not myFile[0].endswith(".xlsx"):
                QtWidgets.QMessageBox.critical(self, "Invalid File!", "Invalid File!\n\nThe input file must be XLSX format.", QtWidgets.QMessageBox.Ok)
                self.input_edit.setText("")
                self.input_path = ""
                global_settings.input_path = ""
            else:
    ...
    def find_parameters(self,input_path, output_path, verbose_level, linear_region_tol):
        ...
        global_settings.loading_window.progressBar.setValue(20) ### Update progress Bar
        QtCore.QCoreApplication.processEvents()

        df = raw_df[['Time (min)']+list(col_map['column'])] # Only keep columns noted in column map
        ...
        # format replicates as: # {enzyme_id: {rep1: conc_rate_df, rep2:...}}
    ...
    def read_input(self,input_path):
        if input_path.endswith('xlsx'):
            raw_df = pd.read_excel(input_path, sheet_name='raw_data')
            col_map = pd.read_excel(input_path, sheet_name='column_map')
            conversions = pd.read_excel(input_path, sheet_name='conversion_factors', index_col='name')
        else:
    ...
    def write_report(self,output_path, out_df, replicates, rates_r2, col_map, linear_region):
        ...
        rates_r.name ='rate(mM/s)'
        rates_out = col_map.set_index('column').join(rates_r).join(rates_r2_r).join(linear_region)
    ...
    def find_rates(self,df, linear_region_tol, col_map):
        ...
        linear_region = pd.Series(index=df.columns[1:], name='linear_region(min)')
    ...
    def read_linear_region(self,time, col, col_map):
        start = col_map.loc[ col_map['column'] == col.name, 'start_linear_region'].values[0]
        end = col_map.loc[ col_map['column'] == col.name, 'end_linear_region'].values[0]
    ...
    def split_replicates_and_subtract_blank(self,rates, col_map):
        # format replicates as: # {enzyme_id: {rep1: conc_rate_df, rep2:...}}
        replicates = {}
        for enzyme_id in col_map['enzyme_id'].unique():
            enz_map = col_map[col_map['enzyme_id'] == enzyme_id]
            rep_dict = {}
            for rep in enz_map['replicate'].unique():
                rep_map = enz_map[enz_map['replicate'] == rep]
                rep_df = rep_map.set_index('column').join(rates)
                # subtract blank
                blank_value = rep_df[rep_df['substrate_concentration'] == 0]['rate'].values
                rep_df['rate'] = rep_df['rate'] - blank_value
                rep_dict[rep] = rep_df
            replicates[enzyme_id] = rep_dict
        return replicates
    ...
    def get_output_df(self,replicates, conversions):
        n_output_decimals = 4
        out_dict = {}
        for enzyme_id, reps in replicates.items():
            ...
            kcat = vmax/all_rep_df.loc[
                all_rep_df['enzyme_id'].str.contains(enzyme_id).index[0],
                'enzyme_concentration_mM']
            kcatkm = kcat/km
            out_dict[enzyme_id] = {'km (mM)':km, 'vmax (mM/s)':vmax, 'kcat (1/s)':kcat, 'kcat/km (1/s/mM)':kcatkm}
    ...
    def find_km_vmax(self,rep_df):
        ...
        # bounds and inital guesses on km and vmax parameters (mM), from Cell Biology by the Numbers: http://book.bionumbers.org/how-many-reactions-do-enzymes-carry-out-each-second/
        ...
        popt, pcov = curve_fit(self.mm_model, xdata=rep_df['substrate_concentration'],
...
class LoadingWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        uic.loadUi(global_settings.appdir + "/ui_files/loading_window.ui", self)
        self.mwfg = self.frameGeometry()  ##Center window
        self.cp = QtWidgets.QDesktopWidget().availableGeometry().center()  ##Center window
        self.mwfg.moveCenter(self.cp)  ##Center window
        self.move(self.mwfg.topLeft())  ##Center window
...
class ResultsWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        uic.loadUi(global_settings.appdir + "/ui_files/results_window.ui", self)
    ...
    def generate_plots(self,output_path, rates_out, out_dict):
        ...
        for enzyme_id, params in out_dict.items():
            self.enzyme_ids.append(enzyme_id) # Add enzyme ID to list
            # Initialize widget, canvas, layout, and toolbar for each new enzyme
            ...
            erates = rates_out[rates_out['enzyme_id'] == enzyme_id]
            # Generate figures
            for i, rep in enumerate(erates['replicate'].unique()):
                rerates = erates[erates['replicate']==rep]
                # Get plot variables
                self.e_rates = rerates['rate(mM/s)']
                self.s_conc = rerates['substrate_concentration']
            ...
            label=(r'$\bf{}:$'.format("{" + enzyme_id.replace("_","-") + "}")+'\nK$_m$={:.3fP}\nV$_{{max}}$={:.3fP}'.format(km,vmax)))

#                     label=r'$\bf{{enzyme_id}}$' + ' Model:\n km={:.1f}\n vmax={:.4f}'.format(km,vmax))
...
#            temp_canvas.axes.legend(loc='center left', bbox_to_anchor=(1, 0.5),fontsize=8)
            ...
            temp_canvas.axes.set_xlabel('Initial substrate (mM)')
            temp_canvas.axes.set_ylabel('Rate (mM/s)')
...
#            self.widget_canvas.axes.legend(loc='center left', bbox_to_anchor=(1, 0.5),fontsize=8)
            ...
            self.widget_canvas.axes.set_xlabel('Initial substrate (mM)')
            self.widget_canvas.axes.set_ylabel('Rate (mM/s)')

            # find max rates across ALL enzymes to make all plots on the same scale
            self.widget_canvas.axes.set_ylim([0, max(rates_out['rate(mM/s)'])*1.1])
            self.tabWidget.addTab(temp_widget,enzyme_id) # Add new tab and append tab_canvas to it
...
class name_window(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        uic.loadUi(global_settings.appdir + "/ui_files/name_window.ui", self)
